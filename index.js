    ...
    client.user.setPresence({
	  ...
	  const mikasa = client.emojis.cache.get("1066189968722120835")
		client.on("messageCreate",  (message) =>{ 
			...
			message.channel.send( (`${mikasa}`)  + "`Prontinho pra tocar sua musica`" +  (`${levi}`) );
		...
		client.on("messageCreate", (message) =>{
			...
			if(message.content === prefix + "ping") message.channel.send ((`${um}`) + `**TATAKAE ** o ping é aproximadamente **${client.ws.ping}ms**` + (`${um}`));
		})
...
client.login("MTA2NTk5MTQ1Njc5MzMwOTIwNA.Gx31Kj.tyBYW5NqQaNJZmUghFiOFXFyeLSHxiqgXtuufY");